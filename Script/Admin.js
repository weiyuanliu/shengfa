...
//删除各种分类节点--------------------------------------------------------------
function ConfirmDelSort(Result,ID,tbs,tb)
//删除产品分类节点--------------------------------------------------------------
{
   if (confirm("您确实要删除本类、子类及所有下属信息条目吗？"))
   {
      window.location.href=Result+".asp?Action=Del&ID="+ID+"&TbS="+tbs+"&tb="+tb
   } 
}
//分类节点展开和折叠(备用)-------------------------------------------------------------
...
{
   var flag1=false;
   var argValue=document.getElementById("argValue").value;
...
function AddSort(SortName,ID,Path)
{
	...
	window.opener.document.editForm.SortName.value=unescape(SortName);
