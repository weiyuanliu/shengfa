...
Dim dgtime,Sh_Name,Sh_Mobel,Sh_Tel,Sheng,shi,xian,Addres,RecordCounts,ZipCodes,QuType,XOrStr,AddersStr,OrderId,ipadd,HuiKuan
...
if Sheng <> "" then
	AddersStr=Sheng&"省"&shi&"市"
else
	AddersStr=shi&"市"
end if

if request.querystring("l")<>"3DFEED3B7B2697C18AFD1F6625334741" then
if session("firstecode_left")<>request("check_left") then
	response.Write("<script language=javascript>"&vbcrlf)
...
'tel
'dim murl,StartGet,Teltoprovince,Teltocity,Teltocard,Sh_Telto
'murl="http://www.baidu.com/s?wd="&Sh_Tel
'StartGet = getHTTPPage(murl)
'Teltoprovince=Getkey(StartGet,Sh_Tel&"&quot;</span>                <span>","</span>")
...
'murlip="http://www.baidu.com/s?wd="&ipadd
'StartGetip = getHTTPPage(murlip)
'iptoadd=Getkey(StartGetip,"IP地址:&nbsp;"&ipadd&"</span>","</td>")
'if iptoadd <> "" then
'	ipto=iptoadd
'end if
%>
...
<input type="hidden" name="Sh_ipto" value="<%=sPlace%>" />
  <tr><td colspan='2' class="orderok_t">请仔细核对订单信息</td></tr>
    <TR>
	  ...
	  <TD width="70%">古那迪<input type="hidden" name="ProdName" value="古那迪" /></TD>
    </TR>
    <TR>
    ...
    <TR>
    	...
    	<TD><%=ZipCodes%><input type="hidden" name="ZipCode" id="ZipCode" value="<%=ZipCodes%>" /></TD>
    </TR>
    <TR>
    	<TD align=center>收货地址:</TD>
    	<TD><input type="text" style="width:300px;" name="AddresStr" value="<%=(AddersStr&Addres)%>" /> 地址栏内容可修改</TD>
    </TR>
    <TR>
	...
	<%else%>
    	<%=FormatCurrency(Sum_Memony2(str,0),2,-2)%>元
		<input type="hidden" name="SumMemony" value="<%=Sum_Memony2(str,0)%>" />
	<%end if%>
	</TD>
    </TR>
    <TR>
...
function savedingdan()
	...
	rs("Remark")="免费快递送货上门|"&Trim(Request.Form("HuiKuan")&"|"&Sum_Memony2(str,0))
	end if
