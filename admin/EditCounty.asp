...
'========判断是否具有管理权限
Dim Action,ParentID,ParentID2,Content,AddTime,Px,ID
ID=Trim(Request("ID"))
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  ...
  <tr>
    ...
    <table width="78%" border="0" cellpadding="5" cellspacing="0">
       <form name="AddProvince" id="AddProvince" method="post" action="EditCounty.asp?Action=AddProvince" onSubmit="return CheckValue();">
      <tr>
...
sub SaveProvince()
	...
	if not rs.eof and not rs.bof then
		...
		response.Write("<script langauge=javascript>"&vbcrlf)
			response.Write("alert('对不起，三级信息已存在！');"&vbcrlf)
			response.Write("window.history.go(-1);"&vbcrlf)
		response.Write("</script>"&vbcrlf)
		response.End()
		exit sub
	...
	if ID="" or isnull(ID) or not(IsNumeric(ID)) then
		...
		if rs.eof and rs.bof then
			...
			response.Write("<script language=javascript>"&vbcrlf)
				...
				response.Write("window.location.href='County.asp';")
			response.Write("</script>"&vbcrlf)
			response.End()
			exit sub
		end if
