...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  <form action="DelContent.asp?Result=About" method="post" name="formDel" >
    <tr>
      ...
      <td width="88" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>查看组别</strong></font></td>
      <td width="52" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>权限方式</strong></font></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td width='48' nowrap><a href='AboutEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改企业信息"")'><font color='#330099'>改</font></a>.<a href='AboutList.asp?Result=ModifySequence&ID="&rs("ID")&"' onClick='changeAdminFlag(""排序企业信息"")'><font color='#330099'>排序</font></a></td>" & vbCrLf
    ...
    Response.Write "<td colspan='2' nowrap  bgcolor='#EBF2F9'><input name='submitDelSelect' type='button' class='button'  id='submitDelSelect' value='删除所选' onClick='ConfirmDel(""您真的要删除这些企业信息吗？"");'></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
    response.write "<tr><td height='50' align='center' colspan='12' nowrap  bgcolor='#EBF2F9'>暂无企业信息</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
...
sub ModifySequence()
  ...
  response.write "<tr>"
  response.write "<td height='24' align='center' nowrap  bgcolor='#EBF2F9'>ID：<input name='ID' type='text' class='textfield'  style='WIDTH: 30;' value='"&ID&"' maxlength='4' readonly>&nbsp;企业信息名称：<input name='AboutName' type='text' class='textfield' id='AboutName' style='WIDTH: 180;' value='"&AboutName&"' maxlength='36' readonly>&nbsp;排序号：<input name='Sequence' type='text' class='textfield' style='WIDTH: 60;' value='"&Sequence&"' maxlength='4'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&Sequence&"';}"">&nbsp;&nbsp;<input name='submitSequence' type='submit' class='button' value='保存' style='WIDTH: 60;' ></td>"
  response.write "</tr>"
  response.write "</form>"
