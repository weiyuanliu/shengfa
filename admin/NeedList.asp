...
function PlaceFlag()
  if Result="Search" then
    Response.Write "需求：列表&nbsp;->&nbsp;检索&nbsp;->&nbsp;添加时间[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，关键字[<font color='red'>"&Keyword&"</font>]"
  else
    if SortPath<>"" then
      Response.Write "需求：列表&nbsp;->&nbsp<a href='NeedList.asp'>全部</a>"
	  TextPath(SortID)
	else
      Response.Write "需求：列表&nbsp;->&nbsp全部"
	end if
  end if
end function  
%>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  ...
  <tr>
    <td height="36" align="center" nowrap  bgcolor="#EBF2F9"><table width="100%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="post" action="Search.asp?Result=Need">
          <td nowrap> 需求检索：从
        ...
        <td align="right" nowrap>查看：<a href="NeedList.asp" onClick='changeAdminFlag("新闻列表")'>全部需求</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="NeedSort.asp" onClick='changeAdminFlag("选择查看分类")'>其他类别需求</a></td>
      </tr>
    </table>      </td>    
  </tr>
</table>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  <form action="DelContent.asp?Result=Need" method="post" name="formDel" >
  <tr>
    ...
    <td nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>需求名称</strong>（指向显示所属类别）</font></td>
    ...
    <td width="118" nowrap bgcolor="#8DB5E9"><strong><font color="#FFFFFF">结束日期<br>
    </font></strong></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
	  ...
	  if StrLen((rs("NeedName")))>36 then
        Response.Write "<td nowrap title='类别：&#13;"&SortText(rs("SortID"))&"&nbsp;|&nbsp;"&rs("SortPath")&"'>"&StrLeft(rs("NeedName"),33)&"</td>" & vbCrLf
      else
        Response.Write "<td nowrap title='类别：&#13;"&SortText(rs("SortID"))&"&nbsp;|&nbsp;"&rs("SortPath")&"'>"&rs("NeedName")&"</td>" & vbCrLf
      end if 
      Response.Write "<td nowrap>" & vbCrLf
      if rs("UrgentFlag") then Response.Write "<font color='red'>急</font>"
	  Response.Write "</td>"
      ...
      Response.Write "<td width='48' nowrap><a href='NeedEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改需求信息"")'><font color='#330099'>修改</font></a></td>" & vbCrLf
    ...
    Response.Write "<td colspan='2' nowrap  bgcolor='#EBF2F9'><input name='submitDelSelect' type='button' class='button'  id='submitDelSelect' value='删除所选' onClick='ConfirmDel(""您真的要删除这些需求吗？"");'></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
    response.write "<tr><td height='50' align='center' colspan='12' nowrap  bgcolor='#EBF2F9'>暂无需求信息</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
...
Function TextPath(ID)
  ...
  rs.open sql,conn,1,1
  TextPath="&nbsp;->&nbsp;<a href=NeedList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&">"&rs("SortName")&"</a>"
