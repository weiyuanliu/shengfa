...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From NwebCn_ProductSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from  NwebCn_ProductSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from  NwebCn_Products where Instr(SortPath,'"&SortPath&"')>0")
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_ProductSort where ParentID="&id&" order by id"
  rs.open sql,conn,1,1
  ...
  while not rs.eof
    ChildCount=conn.execute("select count(*) from NwebCn_ProductSort where ParentID="&rs("id"))(0)
    ...
    response.write("<font color='#FF0000'>分类：</font><a href='ProductSort.asp?Action=Add&ParentID="&rs("id")&"'>添加</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='ProductSort.asp?Action=Edit&ID="&rs("id")&"'>修改</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='ProductSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortName="&rs("SortName")&"&SortPath="&rs("SortPath")&"'>移</a>")
    ...
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;<font color='#FF0000'>产品：</font><a href='ProductEdit.asp?Result=Add' onClick='changeAdminFlag(""添加产品"")'>添加</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='ProductList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""产品列表"")'>列表</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td>排序："&px)
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From NwebCn_ProductSort where ID="&ParentID
    rs.open sql,conn,1,1
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td nowrap>父类数字路径：<input readonly name="ParentPath" type="text" class="textfield" id="ParentPath" size="22" value="<%=ParentPath%>"></td>
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then 
    sql="Select * From NwebCn_ProductSort"
  ...
  rs.update 
  response.write ("<script language=javascript> alert('"&Action&"保存成功，点击确定查看类别树！');location.replace('ProductSort.asp');</script>")
End Function 
'修改节点---------------------------------
Function editFolder()
  Dim ID
...
<form name="FolderForm" method="post" action="ProductSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
  <tr>
    <td height="24" nowrap><font color="#FFFFFF"><img src="Images/Explain.gif" width="18" height="18" border="0" align="absmiddle">&nbsp;<strong>修改类别：通过"发布"可控制每种分类是否在相应语言版网站里显示出来。</strong></font></td>
  </tr>
  <tr>
    <td height="24" nowrap bgcolor="#EBF2F9">|&nbsp;根类&nbsp;→&nbsp;<% if ParentID<>0 then TextPath(ParentID)%></td>
  </tr>
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="120" nowrap>父类ID：<input readonly name="ParentID" type="text" class="textfield" id="ParentID" size="6" value="<%=ParentID %>"></td>
        <td nowrap>父类数字路径：<input readonly name="SortPath" type="text" class="textfield" id="SortPath" size="22" value="<%=SortPath%>"></td>
		 <td nowrap>排序：<input  name="px" type="text" class="textfield" id="px" size="5" value="<%=px%>"></td>
	  </tr>
      <tr>
        <td colspan="5" align="center" height="30" valign="bottom" nowrap><input name="submitSave" type="submit" class="button" id="保存" value="  保存  "></td>
	  </tr>
    </table>
	</td>
  </tr>
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_ProductSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update NwebCn_ProductSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新类别数字路径
  conn.execute("update NwebCn_ProductSort set ParentID='"&toID&"' where ID="&fromID)'更新类别父类ID
  conn.execute("update NwebCn_Products set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新信息数字路径
  response.write ("<script language=javascript> alert('移动类别成功，点击确定查看类别树！');location.replace('ProductSort.asp');</script>")
End Function
%>