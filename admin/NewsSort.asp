...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From NwebCn_NewsSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from  NwebCn_NewsSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from  NwebCn_News where Instr(SortPath,'"&SortPath&"')>0")
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_NewsSort where ParentID="&id&" order by id"
  rs.open sql,conn,1,1
  ...
  while not rs.eof
    ChildCount=conn.execute("select count(*) from NwebCn_NewsSort where ParentID="&rs("id"))(0)
    ...
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='NewsList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""新闻列表"")'>列表</a><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;排序："&px)
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From NwebCn_NewsSort where ID="&ParentID
    rs.open sql,conn,1,1
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_NewsSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update NwebCn_NewsSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新类别数字路径
  conn.execute("update NwebCn_NewsSort set ParentID='"&toID&"' where ID="&fromID)'更新类别父类ID
  conn.execute("update NwebCn_News set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新信息数字路径
