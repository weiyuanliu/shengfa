...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From NwebCn_OthersSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from  NwebCn_OthersSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from  NwebCn_Others where Instr(SortPath,'"&SortPath&"')>0")
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_OthersSort where ParentID="&id&" order by id"
  rs.open sql,conn,1,1
  ...
  while not rs.eof
    ChildCount=conn.execute("select count(*) from NwebCn_OthersSort where ParentID="&rs("id"))(0)
    ...
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='OthersSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortName="&rs("SortName")&"&SortPath="&rs("SortPath")&"'>移</a>")
    ...
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='OthersList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""信息列表"")'>列表</a><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;排序："&px)
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From NwebCn_OthersSort where ID="&ParentID
    rs.open sql,conn,1,1
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then 
    sql="Select * From NwebCn_OthersSort"
  ...
  rs.update 
  response.write ("<script language=javascript> alert('"&Action&"保存成功，点击确定查看类别树！');location.replace('OthersSort.asp');</script>")
End Function 
'修改节点---------------------------------
Function editFolder()
  Dim ID
...
<form name="FolderForm" method="post" action="OthersSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
  <tr>
    <td height="24" nowrap><font color="#FFFFFF"><strong>修改类别：通过"发布"可控制每种分类是否在相应语言版网站里显示出来。</strong></font></td>
  </tr>
  <tr>
    <td height="24" nowrap bgcolor="#EBF2F9">|&nbsp;根类&nbsp;→&nbsp;<% if ParentID<>0 then TextPath(ParentID)%></td>
  </tr>
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_OthersSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update NwebCn_OthersSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新类别数字路径
  conn.execute("update NwebCn_OthersSort set ParentID='"&toID&"' where ID="&fromID)'更新类别父类ID
  conn.execute("update NwebCn_Others set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新信息数字路径
  response.write ("<script language=javascript> alert('移动类别成功，点击确定查看类别树！');location.replace('OthersSort.asp');</script>")
End Function
%>