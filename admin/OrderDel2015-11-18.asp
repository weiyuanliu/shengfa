...
<br>
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  <form name="editForm" method="post" action="OrderDel.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>" onsubmit="return checkChinese();">
  <input name="Fax" type="hidden" id="Fax" value="1">
  <tr>
    <td height="24" nowrap bgcolor="#EBF2F9"><table width="100%" border="0" cellpadding="0" cellspacing="0" id=editProduct idth="100%">
      ...
      <tr>
        <td height="20" align="right">删除原因：</td>
        <td valign="bottom">
...
{ 
if (StrForCheck.length==0) {alert("删除原因不能为空！");return false;} 
} 
else 
{return true;} 
} 
//--> 
      ...
      <tr>
        ...
        <td valign="bottom"><label>
          <input type="submit" name="Modify" id="Modify" value="确定删除">
...
sub OrderEdit()
  ...
  if Action="SaveEdit" then '保存编辑管理员信息
	...
	rs.close
    set rs=nothing 
    response.write "<script language=javascript> alert('删除成功！');changeAdminFlag('订单信息列表');location.replace('OrderList.asp');</script>"
  else '提取留言信息
