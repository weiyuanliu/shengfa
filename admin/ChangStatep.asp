...
<%
'========判断是否具有管理权限
Dim selectID,States,i,YeID,shu
selectID=request.Form("selectID")

YeID=Split(selectID,",") 
shu=0
for i=0 to ubound(YeID)

States=Trim(Request.QueryString("State"))

if YeID(i)="" or isnull(YeID(i)) or not(IsNumeric(YeID(i))) then
	...
	sql="select * from NwebCn_Order where id="&YeID(i)
