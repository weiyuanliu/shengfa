...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  <form action="DelContent.asp?Result=Administrators" method="post" name="formDel" >
    <tr>
      ...
      <td width="52" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>管理员组</strong></font></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    sql="select [ID],[GroupID],[GroupName],[Working],[AdminName],[UserName],[AdminPurview],[Explain],[LastLoginTime],[LastLoginIP],[AddTime]from ["& datafrom &"] where id in("& sqlid &") "&taxis
    set rs=server.createobject("adodb.recordset")
    rs.open sql,conn,0,1
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td nowrap align='center'>"&rs("GroupName")&"</td>" & vbCrLf
